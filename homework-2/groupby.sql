...
SELECT first_name, last_name, phone FROM employees WHERE region IS NULL;
...
SELECT ship_country, SUM(freight) AS total_freight FROM orders WHERE ship_region IS NOT NULL GROUP BY ship_country HAVING SUM(freight) > 2750 ORDER BY total_freight DESC;
